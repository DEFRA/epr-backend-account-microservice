...
namespace BackendAccountService.Api.Controllers
{
    ...
    public class LocalAuthorityOrganisationController : ApiControllerBase
    {
        ...
        {
            _logger.LogInformation("Getting all local authority with district code {DistrictCode}", districtCode);
            ...
            _logger.LogInformation("Error getting all local authority with district code {DistrictCode}", districtCode);
        ...
        {
            _logger.LogInformation("Getting all local authority with name {OrganisationName}", name);
            ...
            _logger.LogInformation("Error getting all local authority with name {OrganisationName}", name);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Getting all local authority with organisation type {OrganisationTypeId}", id);
            if (id < 0)
            {
                return new StatusCodeResult((int)HttpStatusCode.BadRequest);
            }
            ...
            _logger.LogInformation("Error getting all local authority with organisation type {OrganisationTypeId}", id);
            return NotFound(result);
        }

        [HttpGet("external-id")]
        [ProducesResponseType(typeof(LocalAuthorityResponseModel), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(LocalAuthorityResponseModel), StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetLocalAuthorityOrganisationByExternalId(string id)
        {
            _logger.LogInformation("Getting all local authority with external id {ExternalId}", id);
            ...
            _logger.LogInformation("Error getting all local authority with external id {ExternalId}", id);
        ...
        {
            _logger.LogInformation("Creating the selected local authority {District}", request.DistrictCode);
            var result = await _localAuthorityService.CreateNewLocalAuthorityOrganisationAsync(request);

            if (result.IsSuccess)
            {
                _logger.LogInformation("Created the selected local authority Id: {District}", request.DistrictCode);
                return Ok(result);
            }

            _logger.LogInformation("Failed to create the selected local authority {District}", request.DistrictCode);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Updating the selected local authority ExternalId:{ExternalId}", request.ExternalId);
            ...
            _logger.LogInformation("Failed to update the selected local authority ExternalId:{ExternalId}", request.ExternalId);
            return result.BuildErrorResponse();
        }
        ...
        public async Task<IActionResult> UpdateLocalAuthorityOrganisationByDistrictCode(
            UpdateLocalAuthorityRequest request)
        {
            _logger.LogInformation("Updating the selected local authority DistrictCode:{DistrictCode}", request.DistrictCode);
            ...
            _logger.LogInformation("Failed to update the selected local authority DistrictCode:{DistrictCode}", request.DistrictCode);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Removing the selected local authority ExternalId:{ExternalId}", request.ExternalId);
            if (string.IsNullOrWhiteSpace(request.ExternalId) || !Guid.TryParse(request.ExternalId, out _))
            {
                return new StatusCodeResult((int)HttpStatusCode.BadRequest);
            }
            ...
            _logger.LogError("Failed to remove selected local authority ExternalId:{ExternalId}", request.ExternalId);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Removing the selected local authority DistrictCode:{DistrictCode}", request.DistrictCode);
            if (string.IsNullOrWhiteSpace(request.DistrictCode))
            {
                return new StatusCodeResult((int)HttpStatusCode.BadRequest);
            }
            ...
            _logger.LogError("Failed to remove selected local authority DistrictCode:{DistrictCode}", request.DistrictCode);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Getting nation with name {NationName}", nationName);
            ...
            _logger.LogInformation("Error getting nation with name {NationName}", nationName);
            return result.BuildErrorResponse();
        }
        ...
        {
            _logger.LogInformation("Getting organisation type with name {OrganisationTypeName}", organisationTypeName);
            ...
            _logger.LogInformation("Error organisation type with name {OrganisationTypeName}", organisationTypeName);
            return result.BuildErrorResponse();
        }
    }
}